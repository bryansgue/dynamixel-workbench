...
ros::Publisher dynamixel_data_pub_;
...
void DynamixelController::initPublisher()
{
  ...
  if (is_joint_state_topic_) 
    {
      dynamixel_data_pub_ = priv_node_handle_.advertise<sensor_msgs::Joy>("/dynamixel_arm", 10);
  ...
  if (is_cmd_vel_topic_) 
    {
      ...
      cmd_vel_sub_2 = priv_node_handle_.subscribe("cmd_vel_", 10, &DynamixelController::vel_control_callback, this);
    }
}
...
void DynamixelController::publishCallback(const ros::TimerEvent&)
{
  ...
  if (is_joint_state_topic_)
  {
    ...
    for (auto const& dxl:dynamixel_)
    {
      ...
      //controlPosYaw.axes.push_back(velocity);
      id_cnt++;

    }

    //sensor_msgs::Joy controlPosYaw;
    //dynamixel_data_pub_.publish(controlPosYaw);
...
void DynamixelController::vel_control_callback(const sensor_msgs::Joy::ConstPtr& msg) {
  
  velocidad = *msg;

  //std::cout << velocidad.axes[0] << std::endl;
  //std::cout << velocidad.axes[1] << std::endl;
  ...
  {
    if (angular_velocity_1 == 0.0f) 
      dynamixel_velocity[motor_1] = 0;
      else if (angular_velocity_1 < 0.0f)  
        dynamixel_velocity[motor_1] = ((-1.0f) * angular_velocity_1 * 1) + 1023;
      else if (angular_velocity_1 > 0.0f)  
        dynamixel_velocity[motor_1] = (angular_velocity_1 * 1);

    if (angular_velocity_2 == 0.0f) 
      dynamixel_velocity[motor_2] = 0;
      else if (angular_velocity_2 < 0.0f)  
        dynamixel_velocity[motor_2] = ((-1.0f) * angular_velocity_2 * 1) + 1023;
      else if (angular_velocity_2 > 0.0f)  
        dynamixel_velocity[motor_2] = (angular_velocity_2 * 1);


    if (angular_velocity_3 == 0.0f) 
      dynamixel_velocity[motor_3] = 0;
      else if (angular_velocity_3 < 0.0f)  
        dynamixel_velocity[motor_3] = ((-1.0f) * angular_velocity_3 * 1) + 1023;
      else if (angular_velocity_3 > 0.0f)  
        dynamixel_velocity[motor_3] = (angular_velocity_3 * 1);

    if (angular_velocity_4 == 0.0f) 
      dynamixel_velocity[motor_4] = 0;
...
int main(int argc, char **argv)
{
  ...
  //ros::NodeHandle nh;
  //ros::Subscriber displayModeSub = nh.subscribe("cmd_vel_arm", 10, &vel_control_callback);
