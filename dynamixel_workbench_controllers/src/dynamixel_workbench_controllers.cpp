...
sensor_msgs::Joy velocidad;
Eigen::VectorXd qp_ref(4);
...
void DynamixelController::initSubscriber()
{
  ...
  if (is_cmd_vel_topic_) cmd_vel_sub_ = priv_node_handle_.subscribe("/joy", 10, &DynamixelController::commandVelocityCallback, this);
}
...
void DynamixelController::commandVelocityCallback(const sensor_msgs::Joy::ConstPtr& msg)
{
  velocidad = *msg;
  qp_ref << msg->axes[0], msg->axes[1], msg->axes[2], msg->axes[3];
  ...
  {
    std::cout << "Angular Velocities greater than 0.2:" << std::endl;
    if (angular_velocity_1 > 0.2)
      std::cout << "angular_velocity_1: " << angular_velocity_1 << std::endl;
    if (angular_velocity_2 > 0.2)
      std::cout << "angular_velocity_2: " << angular_velocity_2 << std::endl;
    if (angular_velocity_3 > 0.2)
      std::cout << "angular_velocity_3: " << angular_velocity_3 << std::endl;
    if (angular_velocity_4 > 0.2)
      std::cout << "angular_velocity_4: " << angular_velocity_4 << std::endl;
  }
  ...
  int pulses_per_second_1 = angular_velocity_1 * (4095 / (2 * M_PI));
  int pulses_per_second_2 = angular_velocity_2 * (4095 / (2 * M_PI));
  int pulses_per_second_3 = angular_velocity_3 * (4095 / (2 * M_PI));
  int pulses_per_second_4 = angular_velocity_4 * (4095 / (2 * M_PI));


  if (dxl_wb_->getProtocolVersion() == 1.0f)
  {
    if (pulses_per_second_1 == 0.0f) dynamixel_velocity[motor_1] = 0;
    else if (pulses_per_second_1 < 0.0f) dynamixel_velocity[motor_1] = ((-1.0f) * pulses_per_second_1 * 1)  + 1023;
    else if (pulses_per_second_1 > 0.0f) dynamixel_velocity[motor_1] = (pulses_per_second_1 * 1);

    if (pulses_per_second_2 == 0.0f) dynamixel_velocity[motor_2] = 0;
    else if (pulses_per_second_2 < 0.0f) dynamixel_velocity[motor_2] = ((-1.0f) * pulses_per_second_2 * 1)  + 1023;
    else if (pulses_per_second_2 > 0.0f) dynamixel_velocity[motor_2] = (pulses_per_second_2 * 1);

   if (pulses_per_second_3 == 0.0f) dynamixel_velocity[motor_3] = 0;
    else if (pulses_per_second_3 < 0.0f) dynamixel_velocity[motor_3] = ((-1.0f) * pulses_per_second_3 * 1)  + 1023;
    else if (pulses_per_second_3 > 0.0f) dynamixel_velocity[motor_3] = (pulses_per_second_3 * 1);

    if (pulses_per_second_4 == 0.0f) dynamixel_velocity[motor_4] = 0;
    else if (pulses_per_second_4 < 0.0f) dynamixel_velocity[motor_4] = ((-1.0f) * pulses_per_second_4 * 1)  + 1023;
    else if (pulses_per_second_4 > 0.0f) dynamixel_velocity[motor_4] = (pulses_per_second_4 * 1);

  }

  //std::cout << qp_ref << std::endl;
...
//        ros::spinOnce();
  ...
  //      bool result = false;
    //    const char* log = NULL;
      //  std::cout << dynamixel_.size() << std::endl;
